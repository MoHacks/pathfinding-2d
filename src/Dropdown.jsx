...
import { MAX_TOTAL_COL, MAX_TOTAL_ROW, totalCols, totalRows} from './PathFindingAlgorithm';

const StartRowDropdown = ({ onSelectStartCol, onSelectStartRow, onSelectEndCol, onSelectEndRow, 
                           dropDownType, updateStartColPositionInGrid, updateStartRowPositionInGrid,
                           updateEndColPositionInGrid, updateEndRowPositionInGrid,
                           updateGridColSize, updateGridRowSize }) => {
  ...
  const [selectedStartColValue, setSelectedStartColValue] = useState(onSelectStartCol); 
  const [selectedStartRowValue, setSelectedStartRowValue] = useState(onSelectStartRow);
  const [selectedEndColValue, setSelectedEndColValue] = useState(onSelectEndCol);
  const [selectedEndRowValue, setSelectedEndRowValue] = useState(onSelectEndRow);
  ...
  const [selectedGridRowValue, setSelectedGridRowValue] = useState(MAX_TOTAL_ROW);
    ...
    useEffect(() => {
      ...
      console.log("In useEffect(), selectedStartColValue: ", selectedStartColValue);
      console.log("In useEffect(), selectedStartRowValue: ", selectedStartRowValue);
      ...
      if(dropDownType === "start-col-dropdown"){
        console.log("HANDLESTARTCOLCHANGEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: ", 
                 onSelectStartCol, selectedStartColValue, selectedEndColValue, selectedStartRowValue, selectedEndRowValue)
        ...
        console.log("selectedStartColValue after renderDropdownDiv was finished: ", selectedStartColValue);
      }
 
      else if(dropDownType === "start-row-dropdown"){
        console.log("HANDLESTARTROWCHANGEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: ", 
                 onSelectStartRow, selectedStartRowValue, selectedEndRowValue, selectedStartColValue, selectedEndColValue)
        ...
        console.log("selectedStartRowValue after renderDropdownDiv was finished: ", selectedStartRowValue);
      }

      else if(dropDownType === "end-col-dropdown"){
        console.log("HANDLEENDCOLCHANGEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: ", 
                 onSelectEndCol, selectedEndColValue, selectedStartColValue, selectedStartRowValue, selectedEndRowValue)
        console.log("selectedEndColValue BEFORE setSelectedEndColValue:", selectedEndColValue)
        updateEndColPositionInGrid(selectedEndColValue);
        console.log("Within useEffect(), finished updateEndCol");
        
        console.log("selectedEndColValue after renderDropdownDiv was finished: ", selectedEndColValue);
       
        console.log("selectedEndRow after renderDropdownDiv was finished: ", selectedEndRowValue);
        console.log("selectedEndCol after renderDropdownDiv was finished: ", selectedEndColValue);
        console.log("onSelectEndCol AFTERRRR renderDrowndownDiv was finished: ", onSelectEndCol);
        console.log("onSelectStartCol AFTERRRR renderDrowndownDiv was finished: ", onSelectStartCol);
        console.log("onSelectEndRow AFTERRRR renderDrowndownDiv was finished: ", onSelectEndRow);
        console.log("onSelectStartRow AFTERRRR renderDrowndownDiv was finished: ", onSelectStartRow);
      }

      else if(dropDownType === "end-row-dropdown"){
        console.log("HANDLEENDROWCHANGEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE: ", 
                 onSelectEndRow, selectedEndRowValue, selectedStartRowValue, selectedEndColValue, selectedStartColValue)
        ...
        console.log("In useEffect(), finished updateGridCol");
        
      }
      
      else if(dropDownType === "grid-row-dropdown"){
        updateGridRowSize(selectedGridRowValue);
        console.log("In useEffect(), finished updateGridRow");
        
      }

      console.log("in useEffect(), finished renderdropDownDiv");

    }, [selectedStartColValue, selectedStartRowValue, selectedEndColValue, selectedEndRowValue, 
        selectedGridColValue, selectedGridRowValue, onSelectStartCol, onSelectStartRow, 
        onSelectEndCol, onSelectEndRow]);
  

  const handleStartColChange = (event) => {
    const value = parseInt(event.target.value);
    if(value === onSelectEndCol && onSelectStartRow === onSelectEndRow){
      alert("A) Cannot select this Start Node Column Position since it overlaps with the End Node Column Position! Please Select a Different Column.")
    }
    else{
  ...
  const handleStartRowChange = (event) => {
    ...
    if(value === onSelectEndRow && onSelectStartCol === onSelectEndCol){
      alert("B) Cannot select this Start Node Column Position since it overlaps with the End Node Column Position! Please Select a Different Row.")
    }
    else{
  ...
  const handleEndColChange = (event) => {
    ...
    if(value === onSelectStartCol && onSelectEndRow === onSelectStartRow){
      alert("C) Cannot select this Start Node Column Position since it overlaps with the End Node Column Position! Please Select a Different Column.")
    }
    else{
      setSelectedEndColValue(value);
    }
    
    console.log("selectedEndColValue AFTER setSelectedEndColValue: ", selectedEndColValue);
  ...
  const handleEndRowChange = (event) => {
    ...
    if (value === onSelectStartRow && onSelectEndCol === onSelectStartCol) {
      alert("D) Cannot select this Start Node Column Position since it overlaps with the End Node Column Position! Please Select a Different Row.")
    }
    else{    
  ...
  const renderDropdownDiv = () => {
    if (dropDownType === "start-col-dropdown") {
      ...
      <div className="start-col-dropdown">
        ...
        <select id="startCol" value={selectedStartColValue} onChange={handleStartColChange} dropDownType={dropDownType}>
          {Array.from({ length: totalCols}, (_, index) => index + 1).map((col) => (
      ...
      return(
        <div className="start-row-dropdown">
          ...
          <select id="startRow" value={selectedStartRowValue} onChange={handleStartRowChange} dropDownType={dropDownType}>
            {Array.from({ length: totalRows}, (_, index) => index + 1).map((row) => (
      ...
      return (
        <div className="end-col-dropdown">
          ...
          <select id="endCol" value={selectedEndColValue} onChange={handleEndColChange} dropDownType={dropDownType}>
            {Array.from({ length: totalCols}, (_, index) => index + 1).map((col) => (
              <option key={col - 1} value={col - 1}>
                {col}
              </option>
      ...
      return (
        <div className="end-row-dropdown">
          ...
          <select id="endRow" value={selectedEndRowValue} onChange={handleEndRowChange} dropDownType={dropDownType}>
            {Array.from({ length: totalRows}, (_, index) => index + 1).map((row) => (
              <option key={row - 1} value={row - 1}>
                {row}
              </option>
      ...
      return (
        <div className="grid-col-dropdown">
          ...
          <select id="gridCols" value={selectedGridColValue} onChange={handleGridColChange} dropDownType={dropDownType}>
            {Array.from({ length: MAX_TOTAL_COL}, (_, index) => index + 1).map((col) => (
      ...
      return (
        <div className="grid-row-dropdown">
          ...
          <select id="gridRows" value={selectedGridRowValue} onChange={handleGridRowChange} dropDownType={dropDownType}>
            {Array.from({ length: MAX_TOTAL_ROW - 8}, (_, index) => index + 9).map((row) => (
  ...
  //NOTE: ORDER OF FUNCTIONS BEING CALLED: 1) renderDropdrownDiv() 2) handleEndColChange() 3) useEffect)(), 4) return() in PathfindingAlgorithm.jsx
  return (
    <>
      {console.log("FIRST!")}
      {renderDropdownDiv()}
